...
const postIt = async (tweet: { id: number; content: string }) => {
  ...
  try {
    ...
    const transformedTweets = tweets.map(tweet => ({
      ...
      status: tweet.category === 'generated' ? 'ai-generated' : tweet.category,
    ...
    const generated = transformedTweets.filter(t => t.status === 'ai-generated');
    const scheduled = transformedTweets.filter(t => t.status === 'scheduled');
  ...
  return (
    <div className="min-h-screen bg-white overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* Main Content */}
<div
  className="max-w-6xl mx-auto px-6 py-6 h-[calc(100vh-180px)] overflow-y-auto"
  ref={scrollRef}
...
>
  ...
  {renderTweetSection()}
        ...
        <AnimatePresence>
          {showScrollTop && (
            <motion.button
              className="fixed bottom-6 right-6 p-3 bg-blue-600 text-white rounded-full shadow-lg z-20"
              onClick={scrollToTop}
